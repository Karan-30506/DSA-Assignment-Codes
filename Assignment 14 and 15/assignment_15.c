...
void BFS(Graph g, int startValue) {
...
void DFSofGraph(Graph g, int index, bool *visited) {
    visited[index] = true;
    printf("%d ", g.vertexValues[index]);

    Node* temp = g.adjList[index];
    while(temp) {
        if(!visited[temp->vertex]) {
            DFSofGraph(g, temp->vertex, visited);
        }
        temp = temp->next;
    }
}
...
int main() {
    ...
    printf("8) Breadth First Search Traversal\n");
    printf("9) Depth First Search Traversal\n");
